...
class RideHistoryItemViewHolder(private val binding: RideHistoryCardviewBinding) : RecyclerView.ViewHolder(binding.root) {
    ...
    fun bind(ride: RideHistoryItemDTO, context: Context) {

        binding.travelDistanceTV.text = context.getString(R.string.ride_distance_format_text, ride.distance.toFloat()/100)
        binding.travelTimeTV.text = context.getString(R.string.travel_time_title_format_text, ride.duration / 60, ride.duration % 60)
        ...
        binding.startAddressTV.text =
            getScooterAddress(CoordinatesDTO(coordinates = listOf(ride.coordinates[0].longitude, ride.coordinates[0].latitude)), context)
        binding.endAddressTV.text =
            getScooterAddress(CoordinatesDTO(coordinates = listOf(ride.coordinates[1].longitude, ride.coordinates[1].latitude)), context)
